...
type CloudLister interface {
	...
	List(selector labels.Selector) (ret []*topov1alpha1.Cloud, err error)
	// Get retrieves the Cloud from the index for a given name.
	// Objects returned here must be treated as read-only.
	Get(name string) (*topov1alpha1.Cloud, error)
	CloudListerExpansion
}

// cloudLister implements the CloudLister interface.
type cloudLister struct {
	listers.ResourceIndexer[*topov1alpha1.Cloud]
}

// NewCloudLister returns a new CloudLister.
func NewCloudLister(indexer cache.Indexer) CloudLister {
	return &cloudLister{listers.New[*topov1alpha1.Cloud](indexer, topov1alpha1.Resource("cloud"))}
}
